...
declare -A INIR_ONLY_PATHS=(
    ...
    ["${HOME}/.localinir_super_overview_daemon.py"]="iNiR super daemon"
    ...
    ["${HOME}/.localsync-pixel-sddm.py"]="iNiR SDDM theme sync helper"
)
...
package_has_dependents() {
    ...
    case "$distro" in
        ...
        debian|ubuntu)
            ...
            ;;
        nixos)
            # On NixOS, we assume it's managed by the flake.
            # Checking dependencies is complex via CLI, so we assume it might be needed.
            return 1
            ;;
        *)
    ...
    if [[ ${#safe_to_remove[@]} -gt 0 || ${#ask_before_remove[@]} -gt 0 ]]; then
        ...
        case "$distro" in
            ...
            debian|ubuntu)
                ...
                echo -e "  # sudo rm /usr/localqs"
                echo -e "  # sudo rm /usr/localniri"
                echo ""
                ;;
            nixos)
                echo -e "${STY_BLUE}NixOS detected: Dependency management is declarative.${STY_RST}"
                echo -e "To remove iNiR-associated packages, update your flake configuration"
                echo -e "and run: ${STY_BOLD}nixos-rebuild switch${STY_RST}"
                echo ""
                ;;
            *)
