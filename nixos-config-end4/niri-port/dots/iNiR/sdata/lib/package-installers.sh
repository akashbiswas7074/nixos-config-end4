...
install-yay(){
  if [[ "${OS_GROUP_ID}" == "nixos" ]]; then
    log_warning "NixOS detected: Cannot install 'yay'. Manage AUR-like packages via Nix Flake."
    return 0
  fi
...
install-paru(){
  if [[ "${OS_GROUP_ID}" == "nixos" ]]; then
    log_warning "NixOS detected: Cannot install 'paru'. Manage AUR-like packages via Nix Flake."
    return 0
  fi
...
install-python-packages(){
  ...
  if [[ ! -d "$venv_dir/bin" ]]; then
    ...
    if [[ "${OS_GROUP_ID}" == "nixos" ]]; then
      if ! command -v python3 &>/dev/null; then
        log_info "python3 missing, attempting to use nix shell nixpkgs#python3..."
        x nix shell nixpkgs#python3 --command uv venv --prompt ii-venv "$venv_dir" -p python3 || {
          log_warning "Could not create Python venv using nix shell"
          return 0
        }
      else
        python_bin=$(command -v python3)
        x uv venv --prompt ii-venv "$venv_dir" -p "$python_bin" || {
  ...
  if [[ -f "$requirements_file" ]]; then
    ...
    [[ ! -f "$activate_script" ]] && activate_script="$venv_dir/Scripts/activate" # Windows-ish fallback
    
    if [[ -f "$activate_script" ]]; then
      source "$activate_script"
      if [[ "${OS_GROUP_ID}" == "nixos" ]]; then
        # Native Python packages (pycairo/dbus-python/pygobject) need dev
        # pkg-config metadata. `nix shell` alone does not always expose those
        # paths to pkg-config, so we add them explicitly.
        local cairo_dev glib_dev libffi_dev dbus_dev libxcb_dev libX11_dev libXext_dev libXrender_dev xorgproto
        cairo_dev="$(nix build --no-link --print-out-paths nixpkgs#cairo.dev 2>/dev/null | tail -1)"
        glib_dev="$(nix build --no-link --print-out-paths nixpkgs#glib.dev 2>/dev/null | tail -1)"
        libffi_dev="$(nix build --no-link --print-out-paths nixpkgs#libffi.dev 2>/dev/null | tail -1)"
        dbus_dev="$(nix build --no-link --print-out-paths nixpkgs#dbus.dev 2>/dev/null | tail -1)"
        libxcb_dev="$(nix build --no-link --print-out-paths nixpkgs#xorg.libxcb.dev 2>/dev/null | tail -1)"
        libX11_dev="$(nix build --no-link --print-out-paths nixpkgs#xorg.libX11.dev 2>/dev/null | tail -1)"
        libXext_dev="$(nix build --no-link --print-out-paths nixpkgs#xorg.libXext.dev 2>/dev/null | tail -1)"
        libXrender_dev="$(nix build --no-link --print-out-paths nixpkgs#xorg.libXrender.dev 2>/dev/null | tail -1)"
        xorgproto="$(nix build --no-link --print-out-paths nixpkgs#xorg.xorgproto 2>/dev/null | tail -1)"

        local nix_pkg_config_path=""
        [[ -n "$cairo_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${cairo_dev}/lib/pkgconfig"
        [[ -n "$glib_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${glib_dev}/lib/pkgconfig"
        [[ -n "$libffi_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${libffi_dev}/lib/pkgconfig"
        [[ -n "$dbus_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${dbus_dev}/lib/pkgconfig:${dbus_dev}/share/pkgconfig"
        [[ -n "$libxcb_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${libxcb_dev}/lib/pkgconfig"
        [[ -n "$libX11_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${libX11_dev}/lib/pkgconfig"
        [[ -n "$libXext_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${libXext_dev}/lib/pkgconfig"
        [[ -n "$libXrender_dev" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${libXrender_dev}/lib/pkgconfig"
        [[ -n "$xorgproto" ]] && nix_pkg_config_path="${nix_pkg_config_path}:${xorgproto}/lib/pkgconfig"
        nix_pkg_config_path="${nix_pkg_config_path#:}"

        local nix_cpath=""
        [[ -n "$cairo_dev" ]] && nix_cpath="${nix_cpath}:${cairo_dev}/include"
        [[ -n "$glib_dev" ]] && nix_cpath="${nix_cpath}:${glib_dev}/include"
        [[ -n "$libffi_dev" ]] && nix_cpath="${nix_cpath}:${libffi_dev}/include"
        [[ -n "$dbus_dev" ]] && nix_cpath="${nix_cpath}:${dbus_dev}/include"
        [[ -n "$libxcb_dev" ]] && nix_cpath="${nix_cpath}:${libxcb_dev}/include"
        [[ -n "$libX11_dev" ]] && nix_cpath="${nix_cpath}:${libX11_dev}/include"
        [[ -n "$libXext_dev" ]] && nix_cpath="${nix_cpath}:${libXext_dev}/include"
        [[ -n "$libXrender_dev" ]] && nix_cpath="${nix_cpath}:${libXrender_dev}/include"
        ...
        x nix shell \
          ...
          nixpkgs#cairo.dev \
          nixpkgs#glib.dev \
          nixpkgs#xorg.libxcb.dev \
          nixpkgs#xorg.libX11.dev \
          nixpkgs#xorg.libXext.dev \
          nixpkgs#xorg.libXrender.dev \
      ...
      log_warning "Could not find venv activate script"
    fi
  else
...
install-github-binary(){
  ...
  if [[ "${OS_GROUP_ID}" == "nixos" ]]; then
    log_warning "NixOS detected: Direct binary installation of '$name' is discouraged. Add it to your Nix configuration instead."
    return 0
  fi
...
install-starship(){
  ...
  if command -v ~/.localstarship &>/dev/null; then
...
setup-environment-config(){
  ...
  if is_pkg_installed plasma-desktop || is_pkg_installed plasma-workspace; then
    qt_theme="kde"
  fi
...
elif [[ -x ~/.localstarship ]]; then
    eval "$(~/.localstarship init bash)"
fi

# Aliases
if command -v eza &> /dev/null; then
    alias ls='eza --icons'
elif [[ -x ~/.localeza ]]; then
    alias ls='~/.localeza --icons'
fi
alias q='inir run'
...
get-polkit-agent(){
  ...
  # Fallback: check PATH for common agent binaries
  local cmd_agent
  for cmd_agent in polkit-gnome-authentication-agent-1 polkit-kde-authentication-agent-1 polkit-mate-authentication-agent-1 lxpolkit; do
    if command -v "$cmd_agent" &>/dev/null; then
      command -v "$cmd_agent"
      return 0
    fi
  done
