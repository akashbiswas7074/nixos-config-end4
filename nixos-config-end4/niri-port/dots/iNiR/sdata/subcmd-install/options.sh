...
# On NixOS, keep dependencies enabled but skip system-level mutable setup.
is_nixos=false
if [[ "${OS_GROUP_ID:-}" == "nixos" ]] || [[ -f /etc/NIXOS ]] || grep -qi '^ID=nixos' /etc/os-release 2>/dev/null; then
  is_nixos=true
fi
