...
- **ExecStopPost path wrong on repo-sync installs**: service sync only rewrote `ExecStart`, leaving `ExecStopPost` pointing to `inir`. Cleanup-orphans failed silently on every shutdown.
