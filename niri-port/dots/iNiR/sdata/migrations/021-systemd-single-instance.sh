...
migration_check() {
  ...
  local startup_pattern='spawn-at-startup.*(inir([^[:alnum:]_-]|$)|\.localinir|config/quickshell/inir/scripts/inir)'
...
text = path.read_text()
pattern = re.compile(r'.*spawn-at-startup.*(?:inir(?![A-Za-z0-9_-])|\.localinir|config/quickshell/inir/scripts/inir).*\n?', re.MULTILINE)
