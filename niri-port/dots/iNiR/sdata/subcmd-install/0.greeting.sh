...
if [[ "$OS_GROUP_ID" != "arch" && "$OS_GROUP_ID" != "nixos" && "$DETECTED_DISTRO_ID" != "arch" && "$DETECTED_DISTRO_ID" != "endeavouros" && "$DETECTED_DISTRO_ID" != "manjaro" && "$DETECTED_DISTRO_ID" != "garuda" && "$DETECTED_DISTRO_ID" != "cachyos" ]]; then
    distro_warning=$(cat <<EOF
This installer is tuned for Arch-based distributions and NixOS.
