  ...
  *)
    ...
    if [[ -f "$NIRI_CFG" ]]; then
      ...
      if [[ -n "$POLKIT_AGENT" ]]; then
        # Matches any spawn-at-startup line containing "polkit" or the default MATE path
      ...
      if is_pkg_installed plasma-integration; then
      ...
      # NixOS specific environment improvements
      if [[ "${OS_GROUP_ID}" == "nixos" ]]; then
        # 1. Electron/Wayland compatibility
        if ! grep -q "NIXOS_OZONE_WL" "$NIRI_ENV_TARGET"; then
          sed -i '/environment {/a \    NIXOS_OZONE_WL "1"' "$NIRI_ENV_TARGET"
          log_success "NixOS: Added NIXOS_OZONE_WL=1 to Niri environment"
        fi
        
        # 2. Inject profile paths into PATH if Niri doesn't have them
        # (Needed if Niri is launched via systemd/greetd without a full login shell)
        if ! grep -q "PATH" "$NIRI_ENV_TARGET"; then
          local _nix_paths="\$PATH:\$HOME/.nix-profile/bin:/etc/profiles/per-user/\$USER/bin:/run/current-system/sw/bin"
          sed -i "/environment {/a \    PATH \"${_nix_paths}\"" "$NIRI_ENV_TARGET"
          log_success "NixOS: Injected Nix profile paths into Niri environment"
        fi
      fi
...
# Normalize terminal shell command to avoid "Failed to launch child: fish"
# when GUI PATH misses user profile bins (common on NixOS).
normalize_terminal_shells() {
  local fish_path=""
  local bash_path=""
  local foot_conf="${XDG_CONFIG_HOME}/foot/foot.ini"
  local kitty_conf="${XDG_CONFIG_HOME}/kitty/kitty.conf"

  fish_path="$(command -v fish 2>/dev/null || true)"
  bash_path="$(command -v bash 2>/dev/null || echo "/run/current-system/sw/bin/bash")"
  [[ -x "$bash_path" ]] || bash_path="/bin/bash"

  if [[ -f "$foot_conf" ]]; then
    if [[ -n "$fish_path" && -x "$fish_path" ]]; then
      if grep -q '^shell=' "$foot_conf" 2>/dev/null; then
        sed -i "s|^shell=.*$|shell=${fish_path}|" "$foot_conf"
      else
        sed -i "1i shell=${fish_path}" "$foot_conf"
      fi
    else
      if grep -q '^shell=' "$foot_conf" 2>/dev/null; then
        sed -i "s|^shell=.*$|shell=${bash_path}|" "$foot_conf"
      else
        sed -i "1i shell=${bash_path}" "$foot_conf"
      fi
      log_warning "fish not found in PATH, foot fallback set to bash"
    fi
  fi

  if [[ -f "$kitty_conf" ]]; then
    if [[ -n "$fish_path" && -x "$fish_path" ]]; then
      if grep -q '^shell[[:space:]]' "$kitty_conf" 2>/dev/null; then
        sed -i "s|^shell[[:space:]].*$|shell ${fish_path}|" "$kitty_conf"
      else
        sed -i "1i shell ${fish_path}" "$kitty_conf"
      fi
    else
      if grep -q '^shell[[:space:]]' "$kitty_conf" 2>/dev/null; then
        sed -i "s|^shell[[:space:]].*$|shell ${bash_path}|" "$kitty_conf"
      else
        sed -i "1i shell ${bash_path}" "$kitty_conf"
      fi
      log_warning "fish not found in PATH, kitty fallback set to bash"
    fi
  fi
}
normalize_terminal_shells
...
if [[ "${INSTALL_FIRSTRUN}" == true && -n "${DEFAULT_WALLPAPER}" && -f "${DEFAULT_WALLPAPER}" ]]; then
  ...
  export ILLOGICAL_IMPULSE_VIRTUAL_ENV="${INIR_VENV}"
  _init_python_cmd="${INIR_VENV}python3"
