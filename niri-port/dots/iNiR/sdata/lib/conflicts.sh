...
check_conflicts() {
    ...
    for pkg in "${!conflict_map[@]}"; do
        ...
        case "${OS_GROUP_ID:-unknown}" in
            nixos)
                if is_pkg_installed "$pkg"; then installed=true; fi
                ;;
            pacman)
    ...
    if [ ${#conflicts[@]} -gt 0 ]; then
        ...
        if [ ${#critical_conflicts[@]} -gt 0 ]; then
            ...
            if [[ "$package_manager" == "pacman" ]]; then
                ...
                echo -e "${STY_YELLOW}Note: nix-shell and Home Manager might have these installed.${STY_RST}"
                echo -e "Remove them from your flake configuration to avoid conflicts."
            else
