...
# Check if a package is installed (distro-agnostic as much as possible)
function is_pkg_installed() {
  ...
  case "${OS_GROUP_ID:-unknown}" in
    arch)
      ...
      ;;
    nixos)
      # On NixOS, we check if the command exists or if it corresponds to a known system tool.
      case "$pkg" in
        plasma-integration)
          command_exists kreadconfig6 || [[ -n "${QT_PLUGIN_PATH:-}" ]] && return 0
          ;;
        darkly|darkly-qt)
          [[ "${QT_PLUGIN_PATH:-}" == *"darkly"* ]] || [[ -d "$HOME/.local/share/color-schemes" ]] && return 0
          ;;
        *)
          command_exists "$pkg"
          ;;
...
# Run a command, using nix shell nixpkgs#pkg as fallback if the command is missing on NixOS
function nix_run_fallback() {
  local cmd="$1"
  local pkg="$2"
  shift 2
  ...
  if command_exists "$cmd"; then
    ...
    log_info "'$cmd' missing, attempting to use nix shell nixpkgs#$pkg..."
    nix shell "nixpkgs#$pkg" --command "$cmd" "$@"
  else
    log_error "'$cmd' not found and no fallback available."
    return 1
  fi
}
