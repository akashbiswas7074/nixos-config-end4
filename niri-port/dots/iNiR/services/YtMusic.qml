...
Singleton {
    ...
    Process {
        id: _ipcQueryProc
        command: ["sh", "-c", "echo '{ \"command\": [\"get_property\", \"time-pos\"] }' | socat - " + root.ipcSocket + " 2>/dev/null"]
        stdout: SplitParser {
            onRead: line => {
                try {
    ...
    Process {
        ...
        command: ["sh", "-c", "echo '{ \"command\": [\"get_property\", \"pause\"] }' | socat - " + root.ipcSocket + " 2>/dev/null"]
        stdout: SplitParser {
            onRead: line => {
                try {
                    const res = JSON.parse(line)
    ...
    Process {
        id: _ipcEofQueryProc
        command: ["sh", "-c", "echo '{ \"command\": [\"get_property\", \"eof-reached\"] }' | socat - " + root.ipcSocket + " 2>/dev/null"]
        stdout: SplitParser {
            onRead: line => {
                try {
                    const res = JSON.parse(line)
    ...
    Process {
        ...
        command: ["sh", "-c", "echo '" + commandData + "' | socat - " + root.ipcSocket + " 2>/dev/null"]
    }
    ...
    Process {
        id: _oauthDeleteProc
        command: ["sh", "-c", "rm -f \"${XDG_CONFIG_HOME:-$HOME/.config}/illogical-impulse/ytmusic_oauth.json\""]
    }
...
`]
    ...
    Process {
        id: _checkMpvMpris
        command: ["sh", "-c", "test -f /usr/lib/mpv-mpris/mpris.so"]
    ...
    Process {
        ...
        command: ["rm", "-f", root._cookiesFilePath]
    }
    ...
    Process {
        id: _stopProc
        command: ["sh", "-c", "rm -f " + root.ipcSocket]
    }
    ...
    Process {
        ...
        command: ["sh", "-c", "pkill -f 'mpv.*qs-ytmusic-mpv\\.sock' 2>/dev/null; true"]
    }
