...
Singleton {
    ...
    function disableNextTime() {
        Quickshell.execDetached(["sh", "-c", `echo "${root.firstRunFileContent}" > "${root.firstRunFilePath}"`])
    }
    ...
    Process {
        ...
        command: ["sh", "-c", `find "${wallDir}" -maxdepth 1 -type f \\( -name '*.png' -o -name '*.jpg' -o -name '*.jpeg' -o -name '*.webp' \\) 2>/dev/null`]
        stdout: SplitParser {
            onRead: (line) => {
    ...
    Process {
        ...
        onExited: (exitCode) => {
            if (exitCode !== 0) {
                ...
                Quickshell.execDetached(["sh", "-c", `mkdir -p "${parentDir}"`])
