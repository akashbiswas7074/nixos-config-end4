...
Singleton {
    ...
    function convertTheme(sourcePath: string, themeName: string, format: string): void {
        ...
        _runOperation(["gowall", "convert", src, "--theme", themeName, "--output", _previewPathFor(fmt), "--format", fmt], fmt, "", false, _basenameWithoutExt(src))
    }
    ...
    function convertCustomTheme(sourcePath: string, themeName: string, colors, format: string): void {
        ...
        const fmt = _ext(format)
        _runOperation(["gowall", "convert", src, "--theme", _customThemePath, "--output", _previewPathFor(fmt), "--format", fmt], fmt, json, true, _basenameWithoutExt(src))
    }
    ...
    function effectSimple(sourcePath: string, effectName: string, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "effects", effectName, src, "--output", _previewPathFor(fmt)], fmt, "", false, _basenameWithoutExt(src))
    }
    ...
    function effectBrightness(sourcePath: string, factor: real, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "effects", "br", src, "--factor", String(factor), "--output", _previewPathFor(fmt)], fmt, "", false, _basenameWithoutExt(src))
    }
    ...
    function invert(sourcePath: string, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "invert", src, "--output", _previewPathFor(fmt)], fmt, "", false, _basenameWithoutExt(src))
    }
    ...
    function pixelate(sourcePath: string, scale: real, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "pixelate", src, "--scale", String(scale), "--output", _previewPathFor(fmt)], fmt, "", false, _basenameWithoutExt(src))
    }
    ...
    function extract(sourcePath: string, numColors: int): void {
        ...
        if (!_guardReady(src)) return
        extractedColors = []
        extractProc.command = ["gowall", "extract", src, "-c", String(numColors)]
    ...
    function drawBorder(sourcePath: string, borderColor: string, thickness: int, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "draw", "border", src,
    ...
    function drawGrid(sourcePath: string, gridColor: string, gridSize: int, thickness: int, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "draw", "grid", src,
    ...
    function compress(sourcePath: string, quality: int, method: string): void {
        ...
        const cmd = ["gowall", "compress", src, "--quality", String(quality), "--output", outPath]
        if (method.length > 0)
    ...
    function replaceColor(sourcePath: string, fromColor: string, toColor: string, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "convert", src,
    ...
    function upscale(sourcePath: string, scale: int, model: string, format: string): void {
        ...
        if (!_guardReady(src)) return
        const fmt = _ext(format)
        _runOperation(["gowall", "upscale", src,
    ...
    function applyPreview(): void {
        ...
        copyProc.command = ["cp", _currentPreviewPath, dest]
    ...
    Process {
        ...
        command: ["mkdir", "-p", root._runtimeDir, root._outputDir]
        onExited: (exitCode) => {
            ...
            if (root._pendingNeedsThemeFile) {
                ...
                writeThemeProc.command = ["bash", "-c", cmd]
    ...
    Process {
        id: extractProc
        environment: root._gowallEnv
        command: ["gowall", "extract", "/dev/null"]
        stdout: SplitParser {
            onRead: data => {
